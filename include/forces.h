...
/**
 * Adds a force creator to a scene that destroys a bullet when it collides with
 * a tank, and updates the tank's health. The bullet should be destroyed by
 * calling body_remove(). This should be represented as an on-collision callback
 * registered with create_collision().
 *
 * @param scene the scene containing the bodies
...
/**
 * Adds a force creator to a scene that destroys a bullet when it collides with
 * an obstacle The bullet should be destroyed by calling body_remove(). This
 * should be represented as an on-collision callback registered with
 * create_collision().
 *
 * @param scene the scene containing the bodies
 * @param tank the tank that the bullet targets
