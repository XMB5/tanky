...
(function() {
 ...
 var loadPackage = function(metadata) {
  function runWithFS() {
   ...
   do {
    var name_addr = _asan_js_load_4u(start32++);
    var len = _asan_js_load_4u(start32++);
    var content = _asan_js_load_4u(start32++);
    var name = UTF8ToString(name_addr);
    Module["FS_createDataFile"](name, null, HEAP8.subarray(content, content + len), true, true, true);
   } while (_asan_js_load_4u(start32));
  }
  if (Module["calledRun"]) {
   runWithFS();
  } else {
...
var wasmSourceMapFile = "tanky.wasm.map";
...
var ___emscripten_embedded_file_data = Module["___emscripten_embedded_file_data"] = 308566676;
